...
public class ScaraController : MonoBehaviour
{
    ...
    void Update()
    {
        if (currentMode == ControlMode.AutomaticIK) { if (targetObj != null) SolveIK(targetObj.position); }
        else if (currentMode == ControlMode.LinearJog) { HandleLinearJog(); }
        else { ApplyManualControl(); }
    }
